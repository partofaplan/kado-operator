...
helm-crds: manifests ## Sync generated CRDs into the Helm chart.
	@echo "{{- if .Values.installCRDs }}" > charts/kado-operator/templates/crd.yaml
	@echo "# Generated by 'make helm-crds' from config/crd/bases. Do not edit." >> charts/kado-operator/templates/crd.yaml
	@cat config/crd/bases/*.yaml >> charts/kado-operator/templates/crd.yaml
	@echo "{{- end }}" >> charts/kado-operator/templates/crd.yaml
	@echo "Synced CRDs into charts/kado-operator/templates/crd.yaml"

.PHONY: helm-lint
helm-lint: helm-crds ## Lint and render the Helm chart.
	helm lint charts/kado-operator
	helm template kado-operator charts/kado-operator --namespace kado-operator-system > /dev/null
	@echo "Chart renders cleanly."

.PHONY: clean
clean: ## Remove sample resources and CRDs from the cluster, and delete local build output.
...
.PHONY: docker-buildx
docker-buildx: ## Build and push a multi-arch image. The Dockerfile cross-compiles natively.
	- $(CONTAINER_TOOL) buildx create --name kado-operator-builder --driver docker-container
	$(CONTAINER_TOOL) buildx use kado-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag $(IMG) .
	- $(CONTAINER_TOOL) buildx rm kado-operator-builder

.PHONY: build-installer
build-installer: manifests generate kustomize ## Generate a consolidated YAML with CRDs and deployment.
	mkdir -p dist
	cd config/manager && $(KUSTOMIZE) edit set image docker.io/$(IMAGE_NAME)=$(IMG)
